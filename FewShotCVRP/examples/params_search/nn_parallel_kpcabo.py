...
def make_BOSaveSnapshots(bo_torch_module, config_loader):
    @dataclass
    class BOSaveSnapshots(bo_torch_module.KPCABayesianOptimizer):
        cur_root_path: str = ""
        bo_start_time: str = ""
        cvrp_instance_str: str = ""
        ...
        def step(self) -> Tuple[Any, Any]:
            ...
            self.save_snapshot(
                ...
                / f"kpca-bo-{self.bo_iteration_number}"
            )
            best_so_far = float(
                self.y_np_.min().item() if self.minimize else self.y_np_.max().item()
            )
...
def main():
    ...
    cfg = config_loader.load_experiment_config(
        Path(__file__).with_name("kpca-bo-nn-config.json")
    )
    ...
    for cvrp_instance_str in cfg["cvrp_instances"]:
        ...
        bo_cfg = dict(cfg["bo_embedding"])  # shallow copy
        kpcabo_cfg = dict(cfg["kpcabo"])  # shallow copy
