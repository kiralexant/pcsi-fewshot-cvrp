...
from FewShotCVRP.kpcabo.kpca import MyKernelPCA
...
class LinearTransform(PCA):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> LinearTransform:
        ...
        return super().fit(
            X_scaled
        )  # fit the PCA transformation on the scaled data matrix
...
class KernelTransform(MyKernelPCA):
    ...
    def __fit_kernel_parameters(self, X, SearchStrategy):
        ...
        if self.kernel_fit_strategy is KernelFitStrategy.FIXED_KERNEL:
            ...
            raise ValueError(
                f"Kernel fit strategy {str(KernelFitStrategy)} is not known"
            )
...
class KernelParamsSearchStrategy(ABC):
    ...
    def __try_kernel(parameters, epsilon, X_initial_space, X_weighted, kernel_name):
        kpca = MyKernelPCA(
            epsilon,
            X_initial_space,
            {"kernel_name": kernel_name, "kernel_parameters": parameters},
        )
        kpca.fit(X_weighted)
    ...
    @staticmethod
    def __try_rbf_kernel(
        parameters, epsilon, X_initial_space, X_weighted, kernel_name, l2_norm_matrix
    ):
        kpca = MyKernelPCA(
            epsilon,
            X_initial_space,
    ...
    def find_best_for_rbf(self):
        self.__l2_norm_matrix = [
            [np.sum((np.array(a) - np.array(b)) ** 2) for a in self._X_weighted]
            for b in self._X_weighted
        ]
        f = functools.partial(
...
class PCABO(BO):
    ...
    def __init__(self, n_components: Union[float, int] = None, **kwargs):
        ...
        GLOBAL_CHARTS_SAVER = MyChartSaver(
            "PCABO-1", "PCABO", self._search_space.bounds, self.obj_fun
        )
        self.acq_opt_time = 0
    ...
    def _compute_bounds(pca: PCA, search_space: SearchSpace) -> List[float]:
        ...
        radius = np.sqrt(
            np.sum((np.array([l for l, _ in search_space.bounds]) - C) ** 2)
        )
    ...
    def _create_acquisition(
        self, fun=None, par=None, return_dx=False, **kwargs
    ) -> Callable:
    ...
    def ask(self, n_point: int = None) -> List[List[float]]:
        ...
        is_inside = sum(
            ...
            if self.__search_space.bounds[i][0]
            <= new_x[i]
            <= self.__search_space.bounds[i][1]
        ) == len(new_x)
...
class KernelPCABO1(BO):
    ...
    def _compute_bounds(kpca: MyKernelPCA, search_space: SearchSpace) -> List[float]:
        if kpca.kernel_config["kernel_name"] == "rbf":
            ...
            half_diagonal_sqr = sum(
                (ub - mean_i) ** 2 for ((_, ub), mean_i) in zip(search_space.bounds, c)
            )
            ...
            eprintf(
                f"max feature space distance from the mean is {max_features_space_distance_from_mean}"
            )
            C = kpca.transform([c])[0]
            eprintf(f"mean in the feature space is {C}")
            return [
                (
                    _ - max_features_space_distance_from_mean,
                    _ + max_features_space_distance_from_mean,
                )
                for _ in C
            ]
            ...
            max_features_space_distance_from_origin = (
                kpca.kernel(origin, origin)
                + kpca.kernel(corner, corner)
                - 2 * kpca.kernel(origin, corner)
            )
            C = kpca.transform([origin])[0]
            return [
                (
                    _ - max_features_space_distance_from_origin,
                    _ + max_features_space_distance_from_origin,
                )
                for _ in C
            ]
