...
class MyKernelPCA:
    ...
    def f(X, good_subspace, k, V, z_star, bounds, G, w):
        ...
        g_star = MyKernelPCA.__center_g(
            np.array([k(X[i], x_) for i in range(len(X))]), G
        )
        ...
        return sum(
            (np.transpose(np.array(z_star)) - np.matmul(V, np.array(g_star))) ** 2
        ) + np.exp(penalty)
    ...
    def fit(self, X_weighted: np.ndarray):
        ...
        s = 0
        self.k = 0
        while s < (1.0 - self.epsilon) * eignValuesSum:
            s += eignValues[self.k]
            self.k += 1
    ...
    def inverse_transform(self, Y: np.ndarray):
        ...
        for y in Y:
            ...
            w0, fopt, *rest = optimize.fmin_bfgs(
                partial_f, initial_weights, full_output=True, disp=False
            )
